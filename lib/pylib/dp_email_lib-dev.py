...
def fix_delivered_to(file_obj, msg=None, **kw_args):
    ...
    header_status = []  # A Delivered-to header can fuck us in multiple ways.
    m = None
    ...
    if dts and len(dts) > 0:
        ...
        if not m:
            KW_VERBOSE(kw_args, 7, "GASP! A good %s! `%s'\n",
                       DELIVERED_TO_HEADER_NAME, dt0)
            return msg
        # Yep...
    else:
        # No delivered tos at all... we'll make one up.
        header_status.append("none")
        ANOMALY(kw_args, "No %s headers in message.\n",
                DELIVERED_TO_HEADER_NAME)
    
    # Check for an SMTP Received: header and forge a Delivered-To: based on it.
    # Something like:
    # Received: from madness.gogamer.com ([68.225.20.68])
    #   by m1pismtp01-012.prod.mesa1.secureserver.net with SMTP; Fri,
    #   13 Nov 2009 13:00:38 -0700
    # to 
    # else replace newest one.
    new_d_to_header = get_earliest_with_SMTP_for(None, msg=msg, **kw_args).result
    annotation = None
    if not new_d_to_header:
        header_status.append("faked")
        ANOMALY(kw_args, '''No "with E?SMTP for" headers.  Making one up''')
        annotation = ("No %s headers found, found and nothing else was found"
                          + " from which to derive one.",
                      DELIVERED_TO_HEADER_NAME)
    else:
        header_status.append("derived")
        annotation = ("No %s headers found, derived this from E?SMPT Header `%s'",
                      DELIVERED_TO_HEADER_NAME, new_d_to_header)
    new_d_to_header = annotation[0] % annotation[1:]
    if dts:
        # We have a Delivered-To, but it's hosed in some way.
        # I have no idea how this could happen, but the function verifying it
        # can fail.
        header_status.append("replaced")
        msg.replace_header(DELIVERED_TO_HEADER_NAME, new_d_to_header)
    else:
        header_status.append("added")
        msg.add_header(DELIVERED_TO_HEADER_NAME, new_d_to_header)
        
    KW_VERBOSE(kw_args, 3, annotation[0], annotation[1:])
    KW_VERBOSE(kw_args, 3, "header status: %s" % (header_status,))
    annotate_imperfections(msg, X_ANNOTATION_Header, new_d_to_header)
