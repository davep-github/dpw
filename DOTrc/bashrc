...
#
# This isn't really needed except on boxes that are completion able but not
# completion enabled.
...
#    done
    ...
    local bc_type
    local bc
    for bc_dir in /etc /etc/ /etc/bash_completion.d /etc/profile.d $HOME/etc $HOME/.rc
    do
      ...
      do
        ...
        DP_SOURCED_BASH_COMPLETION_FILES="$bc:$bc_type"
