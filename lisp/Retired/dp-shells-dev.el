...
;;;
;;; New design:
;;; This requires my src file patch: 
;;; /home/davep/patches/xemacs/src/21.5.29/keymap.c-added-at-flag.diff
;;; This allows a key press at (point-max) to processed by the extent's key
;;; map.
;;;
;;; Use 2 extents:
;;; 1: (point-min), (comint-bol nil)-1
;;; 2: (comint-bol), (point-max)
;;; XXXXXXXXXXXXXXXXXprompt> command currently being typed in-!-
;;; (point-min)              (comint-bol)                     (point-max)
;;;
;;; This doesn't buy much. I was trying to simplify `dp-shell-xxx-input', but
;;; this only directly handles before-pmark and after-pmark. But not really
;;; that since at-pmark, as depicted above, would really fall into the 2nd
;;; extent, and so would need to be checked for programmatically.
;;;
;;; But the horror that is the current enter key handling needs to be gutted
;;; and reworked.
;;; And serious thought to removing the variant code. This gives (gave?) us
;;; the ability to work with both comint-mode and term-mode. It's been
;;; forever since I've even tried anything but comint-mode.

;;; What was the current enter-key code trying to do?
;;; I think a lot of my problem back then was not knowing enough about
;;; keymaps. One thing in particular was not realizing the the local map is
;;; shared and so all comint-mode buffers, not just the shell-mode ones, see
;;; my changes. I recall being perplexed at the key having been rebound where
;;; it shouldn't've been.

;;; So, I want <enter> to, in comint-mode buffers, do my functions as well as
;;; the pre-existing ones. Some of the modes can have unique send input type
;;; functionality.  In shell buffers, eq, I want to do my own tracking of the
;;; positions at which commands (or other input) has been sent to the
;;; underlying process and then do the execute the original binding.
;;; 
...
(defconst dp-sh-prompt-regexp "^[0-9]+\\([/<][0-9]+\\)?\\([#>]\\)"
  "For bash/sh/etc.")
...
;;wtf was i doing; (defvar dp-shell-vc-commit-cmds '("cvs" "svn" "git" "hg")
;;wtf was i doing;   (loop for vc-cmd in
;;wtf was i doing;     collect (format "\\s-*\\(%s\\|\\S-*/%s\\).* \\<\\(commit\\|ci\\)\\>"
;;wtf was i doing;                     vc-cmd vc-cmd)))
...
This causes an annoying problem in that the TAGS buffers are updated with the
new contents and marked dirty even though the buffer contents matches the
file contents.  This is some new bullshit... it affects reverting buffers for
changed files.  They end up modified.  This didn't use to happen.  IIR, it
was related to stuff like buffer coding systems or some such.")
  ...
  (if (or (not dp-shell-output-max-lines)
    ...
    (if (and dp-shell-output-max-lines
      ...
      (if (and dp-shell-ask-to-limit-comint-output-p
        ...
        (save-excursion
          (dp-end-of-buffer)
          (insert "\n\n*** TOO MANY OUTPUT LINES.  COMMAND ABORTED ***\n\n"))
        (ding)
...
(defun dp-shell-mode-hook ()
  ...
  (unless (dp-shell-ignored-buffer-p (buffer-name))
    ;; Don't do these actions for a certain subset of modes. Eg ftp buffers.
...
(defun dp-shell-type-enter-func-p (func)
  "Is FUNC one of my special <enter> key functions?
They are identified by a property on the function symbol."
...
(defconst dp-shell-output-max-lines-default 7192)
...
`dp-shell-original-enter-binding'."
  ...
  (message "  the C-m binding is: %s" (key-binding "\C-m" t)))
...
return t if we are in a shell type buffer, false otherwise."
  ...
  (let ((is-shell-type (dp-shell-buffer-type (buffer-name))))
    (if is-shell-type
	(progn
          ;; <debug-code>(ebo 'dp-hide-region nil 1)</debug-code> 
	  ...
	  (if (not (compilation-buffer-p (current-buffer)))
		...
		;; this is only needed and correct on the first entry.
                ;; at this time: 2010-08-07T18:23:03, I don't think this is
                ;; correct, since we can enter this mode at any time.
                (goto-char (point-min))
...
@todo Use/write i/f to `previous-error-p' to make us go backwards."
  ...
  (dmessage "@todo Use `previous-error-p' to go back to previous error."))
...
(defun dp-shell-beginning-of-line ()
  ...
  (let ((point (point)))
    (comint-bol nil)                    ; Move to comint's beginning of line.
    (when (= point (point))             ; Did we move?
      (beginning-of-line)               ; No --> we're already there. 
      ;; Skip to next position in sequence. Ie don't go to comint-bol, which,
      ;; seeing as we're there already, is a NOP.
...
(setq shell-dirtrack-process-other-func 'dp-shell-dirtrack-other
      shell-dirtrack-other-regexp "g\\|gb\\|kd")
...
(defsubst dp-shell-xxx-buffer-p(&optional buffer buffer-id-list)
  (let ((isa-shell-buf-p (dp-shell-buffer-p buffer)))
    (and isa-shell-buf-p
         (memq isa-shell-buf-p (or buffer-id-list 
                                   '(shell dp-shell))))))
...
(defun dp-shell-cycle-buffers (&optional other-frame-p buffer)
  (interactive "P")
  (funcall
   (if other-frame-p
