...
;; 3) (format-kbd-macro 'name) on name from step 2 in an elisp interaction
;;    buffer.
...
(defun* dp-new-file-action-by-regexp (pred 
                                      action
                                      &key
                                      pred-args
                                      action-args)
  ...
  (when buffer-file-name
    ...
    (let ((bfn (expand-file-name buffer-file-name)))
      (when (apply pred (cons bfn pred-args))
        (apply action (cons bfn action-args))))))

(defun* dp-force-read-only-by-file-name-regexp ()
  (dp-new-file-action-by-regexp 
