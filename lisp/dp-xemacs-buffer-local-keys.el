...
(message "dp-xemacs-buffer-local-keys loading...")
...
;;;       (dmessage "Whoot! New buffer local key code.")
      ...
      (defun dp-define-buffer-local-keys (keys &optional 
                                          buffer protect-bindings-p name)
          ...
          (let* ((maps (dp-blm-init-buffer))
            ...
            (loop for (key def) on keys by 'cddr do
              ...
              (when (or (not (lookup-key keymap key))
                        (memq protect-bindings-p '(nil over-write)))
...
Called as a oneshot from a buffer-local hook."
  ...
  (defun dp-define-buffer-local-keys (keys &optional keymap buffer copy-p 
                                      &rest rest)
...
(dp-defaliases 'dp-buffer-local-set-keys 'dp-blm-set-keys 
