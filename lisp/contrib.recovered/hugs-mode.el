...
(defvar hugs-dignostic-regexp "\"\\(.*\\)\"\\(( line \\|:\\)\\([0-9]*\\)"
  "Common part of a hugs diagnostic (error|warning) message.")

(defvar hugs-error-regexp (concat "^ERROR " hugs-dignostic-regexp)
  "Error message regular expression.")
(defvar hugs-warning-regexp (concat "^WARNING " hugs-dignostic-regexp)
  "Error message regular expression.")
...
(defconst hugs-emacs-19 
  (and (not hugs-xemacs)
       (progn (string-match "^\\([0-9]+\\)\\." emacs-version)
              (>= (string-to-number (match-string 1 emacs-version)) 19)))
  "t when using Emacs-19 or higher")
  ...
  (let ((index-alist '())
    ...
    (while (> (point) (point-min))
      ...
      (cond ((and (not (looking-at hugs-start-keywords-re))
            ...
            ((re-search-forward "^\\(data\\|type\\|newtype\\)\\>" end t)
...
(defun hugs-kill-emacs-hook ()
  ...
  (if (not (or hugs-emacs-19 hugs-xemacs))
      ;; Run the hook we inherited, if any.
      (and hugs-inherited-kill-emacs-hook
...
point in the errant file."
  ...
  ;; Ensure the Hugs buffer is selected.
  (set-buffer hugs-process-buffer)
  (dmessage "hsebn>%s<" (buffer-name))
  ;; Error message search starts from last load command.
  (goto-char hugs-load-end)
  (if (re-search-forward hugs-error-regexp nil t)
...
(defun hugs-load-message ()
  ...
  (let ((mesg (if (re-search-forward hugs-error-regexp nil t)
                  "There was an error"
    ...
    (if hugs-tipsi-support
        (progn
          ...
          ;; Warning messages search starts from last load command.
          (goto-char hugs-load-end)
          (while (re-search-forward hugs-warning-regexp nil t)
        ...
        ((or (looking-at "^\\sw+")
         ...
         (let ((name (buffer-substring-no-properties (match-beginning 0)
           ...
           (cond ((looking-at "^\\(\\|class\\|instance\\)\\>")
                 ...
                 ((looking-at "^\\(data\\|import\\|type\\|newtype\\)\\>")
...
(defun hugs-find-decl-type (name)
  ...
  (let ((oldpos (point)))
    (goto-char (point-min))
    (or (catch 'find
          (while (re-search-forward "^\\(data\\|type\\|newtype\\)[ \t]+" nil t)
            (re-search-forward "=>[ \t]*" (hugs-get-end-of-line) t)
            (if (looking-at (concat name "\\>"))
                (throw 'find t))))
        (not (goto-char oldpos)))))
...
(if (or hugs-emacs-19 hugs-xemacs)
