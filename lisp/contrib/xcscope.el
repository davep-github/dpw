...
;
; File:         xcscope.el
; RCS:          $RCSfile: xcscope.el,v $ $Revision: 1.2 $ $Date: 2005/12/01 17:36:08 $ $Author: davep $
...
(make-face 'xcscope-arrow-face)
(or (face-differs-from-default-p 'xcscope-arrow-face)
   ;; Usually has a better default value than highlight does
   (copy-face 'isearch 'xcscope-arrow-face))

;;;(defvar xcscope-arrow-extent nil)
(defvar xcscope-arrow-plist nil)
(defvar xcscope-arrow-color 4)
(defconst xcscope-arrow-id 'dp-xcscope-arrow-id)

(defun xcscope-highlight-line (&optional source-buffer pos)
  (interactive)
  (with-current-buffer (or source-buffer (current-buffer))
    (save-excursion
      (when xcscope-arrow-plist
        (xcscope-unhighlight-line))
      (let ((extent (dp-colorize-region xcscope-arrow-color
                                        (line-beginning-position)
                                        (line-end-position)
                                        'NO-ROLL-COLORS-P
                                        'OVERWRITE-P
                                        xcscope-arrow-id t)))
        (setq xcscope-arrow-plist (dp-plist-put 
                                   xcscope-arrow-plist
                                   'ext extent
...
(defun xcscope-unhighlight-line-old ()
  (interactive)
    (when xcscope-arrow-marker
      (dp-unhighlight-point :id-prop dp-xcscope-arrow-id 
                            :pos xcscope-arrow-marker)
      (setq xcscope-arrow-marker nil)))

;;; was      (setq xcscope-arrow-extent (delete-extent xcscope-arrow-extent))))

(defun xcscope-highlight-line-old (&optional source-buffer pos)
  (interactive)
  (setq-ifnil source-buffer (current-buffer)
              pos (point))
  (xcscope-unhighlight-line)
  (setq xcscope-arrow-marker
     (dp-highlight-point :id-prop dp-xcscope-arrow-id 
                         :pos (dp-mk-marker pos source-buffer))))
;     (or extent
;         (save-excursion
;           (with-current-buffer source-buffer
;             (dp-highlight-point pos)
;           (setq extent (make-extent (line-beginning-position)
;                                     (line-end-position)
;                                     source-buffer))
;           (set-extent-face extent 'xcscope-arrow-face)
; 	  ;;(set-extent-begin-glyph extent xcscope-arrow-glyph)
;           ;;(set-extent-begin-glyph-layout extent 'whitespace)
;           (set-extent-priority extent 2000)
;           (setq xcscope-arrow-extent extent)))))
	...
	(progn
	  ...
	  (if (> line-number 0)
		...
		(if (and cscope-allow-arrow-overlays arrow-p)
                    (xcscope-highlight-line (current-buffer) (point))
                  (xcscope-unhighlight-line))
		(or (not save-mark-p)
		    (= old-pos (point))
		    (push-mark old-pos))
...
Push current point on mark ring and select the entry window."
  (interactive)
  (save-excursion
    (beginning-of-line)                 ; dp: So we work at end of line.
...
Point is not saved on mark ring."
  (interactive)
  (save-excursion
    (beginning-of-line)                 ; So we work at end of line.
    (let ((file (get-text-property (point) 'cscope-file))
          (line-number (get-text-property (point) 'cscope-line-number)))
...
(defvar cscope-pre-command-line-options nil
  "Options to insert before the command on the command line.")
(defvar cscope-post-command-line-options nil
  "Options to insert after the command on the command line.")

(defvar cscope-caseless-arg "-C"
  "Option to cscope which causes it to ignore case when searching.")

(defun cscope-caseless ()
  "HACK: make cscope queries ignore case."
  (interactive)
  (add-to-list 'cscope-pre-command-line-options cscope-caseless-arg))

(defun cscope-caseful ()
  "HACK: make cscope queries respect case."
  (interactive)
  (setq cscope-pre-command-line-options 
        (delq cscope-caseless-arg cscope-pre-command-line-options)))
...
SENTINEL-FUNC are optional process filter and sentinel, respectively."
  (setq args (append cscope-pre-command-line-options 
                     args
                     cscope-post-command-line-options))
  (let ( (outbuf (get-buffer-create cscope-output-buffer-name))
	...
	(if (and cscope-marker-window (window-live-p cscope-marker-window))
	    (progn
