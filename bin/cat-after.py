...
def process_file(file_name, regexp, num_matches_to_skip):
    if type(file_name) == type(""):
        fop = open(file_name)
        close_p = True
    else:
        close_p = False
    if num_matches_to_skip < 0:
        print_match_p = True
        num_matches_to_skip = -num_matches_to_skip
    else:
        print_match_p = False
...
# e.g.                          help="Something normal actions can't handle.")
    ...
    if file_names == []:
        dp_io.eprintf("No files specified.\n")
        sys.exit(1)
    ...
    for file_name in file_names:
        process_file(file_name, regexp, app_args.num_matches_to_skip)
