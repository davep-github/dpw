...
true ${outfile:=exported_pass_pwds}

for file in "$prefix"/**/*.gpg; do
    file="${file/$prefix//}"
    printf "%s\n" "Name: ${file%.*}"
    EExec pass-pass "${file%.*}"
    printf "\n\n"
done  >> "${outfile}"
#
{
    echo "you should encrypt ${outfile}"
    echo "piping to pgp/gpg would be betta."
    echo "using /dev/shm would be good."
    echo "using a generated temp file name would give a nano-bit
of security through nano-obscurity."
} 1>&2
