#!/usr/bin/env python2
###
### Update for Python 3, by forcing it to run under Python2.
    ...
    while keep_looping:
        ...
        if dict_file is not False:
            ...
            try:
                ...
                print >>sys.stderr, \
                      "Exception `{}' reading dict file>{}<".format(err, dict_file)
                pyc_file = "%sc" % (dict_file,)
                print >>sys.stderr, "pyc_file>%s<" % (pyc_file)
                os.system("rm -f %s" % (pyc_file,))
                if keep_looping == 1:
                    break
                keep_looping = keep_looping - 1
                print >>sys.stderr, "   removed file and retrying."
                continue
            except Exception, err:
                print >>sys.stderr, \
                "Exception `{}' reading dict file>{}<, trying pickle".format(err,
                                                                             dict_file)
...
if __name__ == "__main__":
    ...
    grep_regexps = []
    opts, args = getopt.getopt(sys.argv[1:], 'efvdlLs:Eq:g:G:m:M:S:pP:aFuUr')
