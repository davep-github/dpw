...
#ifdef   GLIDE_POINT_XLAT
   if (gp_tty_IsTrueGPXlatMode(tty))
   {
      gp_tty_GPXlatData* gpx;
      if ((gpx = gp_tty_FindGPXlatData(tty)) == NULL)
      {
         gp_tty_log("n_tty_rx_buf: find ret: NULL\n);
         return;
      }
      else
      {
         gp_tty_log("n_tty_rx_buf: find ret: 0x%x\n", gpx);
      }

      while(count--)
         gp_tty_GlidePointXlat(tty, gpx, *cp++, *fp++);
