...
#else
         fprintf(LogFile, "%s: MemMapWrite not supported in this environment.\n",
            ProgName);
         exit (1);
#endif
      }
...
long  size)
{
   ...
   if (MemMapWrite)
   {
      ...
      PagerModeAddr = shmat(fd, NULL, SHM_MAP);
...
char  *argv[])
{
   ...
   options = multi ?
      "V:D:F:XrWwa:o:e:s:vb:hlj:d:f:n:piIBqc:t:T:xNRL:S:mA" :
      "V:D:F:XrWwa:o:e:s:vb:hlj:piIBc:t:T:xNRL:S:mA";
