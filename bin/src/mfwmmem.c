...
#define  ANNOYING    (500)
#define  IsANNOYING  (Verbose >= ANNOYING)
#define  VERBOSE     (100)
#define  IsVERBOSE   (Verbose >= VERBOSE)
#define  VERBOSISH   (50)
#define  IsVERBOSISH (Verbose >= VERBOSISH)
...
int   FillNumeric = 0;
int   BLOCKSize = 512;
int   PagerWriteMode = 0;
int   AntiForward = 0;
long  FirstBlockNum = -1;
long  LastBlockNum = 0x7ffffffe;
...
/*int	IOVScramble[MAXIOVs] = {0, 1, 2, 3};*/
...
char     *file)
{
   ...
   if ((buf = alloca(BLOCKSize)) == NULL)
   {
      fprintf(logFile, "%s: alloca failed in FillIDBlock()\n", ProgName);
      exit(1);
   }
...
void  SigHandler (int sig)
{
   ...
   fprintf(logFile, "\n%s(%u): signal %d received on\n%s", ProgName,
      getpid(), sig, ctime(&now));
...
int   IOVInit(long numToAlloc)
{
   ...
   /* assure that we get all vectors filled */
   allocMax = numToAlloc / MAXIOVs;
   ...
   for (i = 0; i < MAXIOVs && numToAlloc != 0; i ++)
   {
      ...
      if (IsANNOYING)
         printf("IOVs[%d]: _base: 0x%08lx, _len: 0x%08lx\n",
            iovNum, IOVs[iovNum].iov_base, IOVs[iovNum].iov_len);
   ...
   if (numIOVs != MAXIOVs)
   {
      fprintf(logFile, "%s: Did not fill all IOVs!\n", ProgName);
      exit(1);
   }
...
int multi)
{
   ...
   fprintf(stderr, "V\tSpecify verbosity of  output... -v is %d.\n",
      VERBOSE);
...
int   fd)
{
   ...
   offset = BLOCKSize * FirstBlockNum;

   if (BytesInBigBuf)
   {
      if (PagerWriteMode)
      {
         memcpy(PagerModeAddr + offset, BigBuf, BytesInBigBuf);
#if 0
         if (msync(PagerModeAddr + offset, BytesInBigBuf, MS_SYNC) != 0)
         {
            fprintf(logFile, "%s: msync failed in FlushBuf() :", ProgName);
            perror("");
            fprintf(logFile, "   PMA: 0x%x, numBytes: 0x%x, flags: 0x%x\n",
               PagerModeAddr+offset, BytesInBigBuf, MS_SYNC);
      ...
      else
      {
         ...
         {
            fprintf(logFile, "%s: seek failed in FlushBuf() :", ProgName);
            perror("");
            exit(1);
         }
         ...
         else
         {
            ...
            {
               fprintf(logFile, "%s: DoWriteV failed in FlushBuf()\n", ProgName);
               fprintf(logFile, "rc: %ld, req: %ld\n", rc, BytesInBigBuf);
               perror("Error");
               exit(1);
            }
         }
...
char  *data)
{
   ...
   if (BytesInBigBuf && FirstBlockNum != -1 &&
         (BigBufp >= BigBuf + BigBufSize ||
...
char     *file)
{
   ...
   if ((id = alloca(BLOCKSize)) == NULL)
   {
      fprintf(logFile, "%s: alloca failed in FillAndWriteBlock()\n", ProgName);
      exit(1);
   }
...
long  size)
{
   ...
   numBytes = BLOCKSize * size;

   if ((id = alloca(BLOCKSize)) == NULL)
   {
      fprintf(logFile, "%s: alloca failed in FunctionalWrite()\n", ProgName);
      exit(1);
   }
   ...
   if ((fd = open(file, O_CREAT | O_TRUNC| O_RDWR, 0666)) < 0)
   {
      fprintf(logFile, "cannot open %s to write:", file);
      perror("");
      exit(1);
   ...
   if (PagerWriteMode)
   {
      if (numBytes)
      {
         if (lseek(fd, numBytes - 1, SEEK_SET) != numBytes - 1)
         {
            fprintf(logFile, "%s: seek failed in FunctionalWrite() :",
               ProgName);
            perror("");
            exit(1);
         }
      ...
      PagerModeAddr = mmap(NULL, numBytes, PROT_READ | PROT_WRITE,
         MAP_VARIABLE | MAP_FILE | MAP_PRIVATE, fd, 0L);
      if (PagerModeAddr == (byte *)-1)
      {
         fprintf(logFile, "%s: mmap failed in FunctionalWrite() :", ProgName);
         perror("");
         exit(1);
      }
      else
      {
         fprintf(stderr, "PagerModeAddr: 0x%x\n", PagerModeAddr);
   ...
   if (AntiForward)
   {
      for (blockNum = size - 1; blockNum >= 0; blockNum--)
      {
   ...
   if (PagerWriteMode)
   {
      if (munmap(PagerModeAddr, numBytes) != 0)
      {
         fprintf(logFile, "%s: munmap failed in FunctionalWrite() :", ProgName);
         perror("");
         exit(1);
      }
...
char     *file)
{
   ...
   if ((id = alloca(BLOCKSize)) == NULL)
   {
      fprintf(logFile, "%s: alloca failed in CompareBuf()\n", ProgName);
      exit(1);
   }
...
long  size)
{
   ...
   if ((id = alloca(BLOCKSize)) == NULL)
   {
      fprintf(logFile, "%s: alloca failed in FunctionalWriteThenRead()\n", ProgName);
      exit(1);
   }
...
void  SigInit(void)
{
   ...
   for (i = 1; i <= SIGMAX; i++)
   {
      switch (i)
      {
         /* keep default handling for these... */
         case SIGKILL:
         case SIGSTOP:
         case SIGCONT:
         case SIGTSTP:
         case SIGSEGV:
#ifdef   SIGWINCH
         case SIGWINCH:    /* pesky window changes */
#endif
            continue;
      }
...
char  *argv[])
{
   ...
   options = multi ?
      "V:D:F:XrWwa:o:e:s:vb:hlj:d:f:n:piIBqc:t:T:xNRL:S:PA" :
      "V:D:F:XrWwa:o:e:s:vb:hlj:piIBc:t:T:xNRL:S:PA";
   ...
   while ((option = getopt(argc, argv, options)) != EOF)
   {
      switch (option)
      {
         ...
         case 'S':
            BLOCKSize = strtol(optarg, &openMode, 0);
            break;
         ...
         case 'V':
            Verbose = strtol(optarg, &openMode, 0);
            break;
